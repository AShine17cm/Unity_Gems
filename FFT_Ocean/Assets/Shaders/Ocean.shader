Shader "Ocean/Ocean"
{
	Properties
	{
		...
		Wave_scale("Wave_scale", Range(1,10)) = 0   //名不副实
		...
		SubShader
	{
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			float4 worldUV = float4(worldPos.xz, 0, 0);//世界坐标 转 UV
			...
			float maxScale = 1.6;//可以让波峰锐利
			float lod_c0 = min(Wave_scale * LengthScale0,maxScale);//大波
			float lod_c1 = min(Wave_scale * LengthScale1,maxScale);//中波
			float lod_c2 = min(Wave_scale * LengthScale2,maxScale);//小波
			...
			displacement += tex2Dlod(_Displacement_c0, worldUV / LengthScale0) * lod_c0;    //大波 250
			largeWavesBias = displacement.y;

			#if defined(MID) || defined(CLOSE)
			displacement += tex2Dlod(_Displacement_c1, worldUV / LengthScale1) * lod_c1;    //中波 17 叠加
			#endif

			#if defined(CLOSE)
			displacement += tex2Dlod(_Displacement_c2, worldUV / LengthScale2) * lod_c2;    //小波 5 叠加
			#endif

			v.vertex.xyz += mul(unity_WorldToObject,displacement);//变换顶点 向量变换到 Local
