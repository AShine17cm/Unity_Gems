...
This script will do the first time setup on the material and then set it to 1. You won't see the value in the editor.
-----------------------------------------
...
public class BlendAddPopup : MaterialPropertyDrawer {
    ...
    public BlendAddPopup(float opaque) {
    ...
    public BlendAddPopup(string src, string dst, string zwrite) {
        _src = src;
        _dst = dst;
        _ZWrite = zwrite;
    }
    ...
    protected void SetupMaterialWithBlendMode(Material material, BlendModes blendMode) {
        switch (blendMode) {
            ...
            case BlendModes.Blend:
                ...
                break;
            case BlendModes.BlendAdd:
                SetBlendAddMode(material);
                break;
        }
    }
    ...
    private void SetBlendAddMode(Material material) {
        material.SetOverrideTag("RenderType", "Transparent");
        material.SetInt(_src, (int) UnityEngine.Rendering.BlendMode.One);
        material.SetInt(_dst, (int) UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
        material.SetInt(_ZWrite, 0);
    ...
    private void SetPremulMode(Material material) {
        ...
        material.DisableKeyword("_ALPHATEST_ON");
        material.renderQueue = CustomQueue(material) ?
