...
public class Func {
    ...
    public static bool Foldout(ref bool display, bool value, bool hasToggle, string title) {
        var style = new GUIStyle {
                                     ...
                                     //fixedHeight = 22,
                                     contentOffset = new Vector2(32f, 0f)
                                 }; // BG

        var rect = GUILayoutUtility.GetRect(16f, 8, style); // Box
        rect.yMin -= 10;
        rect.yMax += 10;
        GUI.Box(rect, "", style);
        ...
        EditorGUI.PrefixLabel(
                              new Rect(
                                       ...
                                       rect.y, 13f, 13f), // title pos
                              new GUIContent(title),
                              titleStyle);

        var triangleRect = new Rect(rect.x + 4f, rect.y + 4, 13f, 13f); // triangle
    ...
    public static void PowerSlider(MaterialProperty prop, float power, Rect position, GUIContent label) {
        ...
        if ((double) power != 1.0) {
            start = PowPreserveSign(start, 1f / power);
            end = PowPreserveSign(end, 1f / power);
        ...
        if (position2.width >= 50f)
            value = GUI.Slider(position2, value, 0.0f, start, end, GUI.skin.horizontalSlider,
                               !EditorGUI.showMixedValue ? GUI.skin.horizontalSliderThumb : "SliderMixed",
                               true, controlId);
    ...
    public static MaterialProperty[] GetProperties(MaterialEditor editor) {
        if (editor.customShaderGUI != null && editor.customShaderGUI is URPGUI) {
            URPGUI gui = editor.customShaderGUI as URPGUI;
            return gui.props;
        }

        Debug.LogWarning("Please add \"CustomEditor \"URPGUI\"\" to the end of your shader!");
        return null;
    }
    ...
    public static bool NeedShow(string group) {
        if (group == "" || group == "_")
            return true;
        if (GUIData.group.ContainsKey(group)) { // 一般sub
