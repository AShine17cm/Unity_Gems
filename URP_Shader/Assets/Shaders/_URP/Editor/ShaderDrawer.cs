...
/// style：0 default collapse；1 expanded；2 collapse without toggle；3 expanded without toggle
/// </summary>
public class MainDrawer : MaterialPropertyDrawer {
    ...
    public MainDrawer(string group, float style) : this(group, "", style) {
    }
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor) {
        ...
        var g = group != "" ? group : prop.name;
        var lastShow = !GUIData.@group.ContainsKey(g) || GUIData.@group[g];
        show = (style == 1 || style == 3) && lastShow || show;

        var result = Func.Foldout(ref show, value, style == 0 || style == 1, label.text);
        ...
        if (group != "" && group != "_") {
            ...
            if (needShow) {
                if (matchPropType)
                    ...
                    Debug.LogWarning($"{GetType()} does not support this MaterialProperty type:'{prop.type}'!");
...
public class KWEnumDrawer : SubDrawer {
    ...
    public KWEnumDrawer(string group, string[] names, string[] keyWords) {
        ...
        values = new int[keyWords.Length];
    ...
    public override void DrawProp(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor) {
        ...
        EditorGUI.showMixedValue = prop.hasMixedValue;
        var num = EditorGUI.IntPopup(rect, label.text, index, names, values);
        EditorGUI.showMixedValue = false;
...
public class TexDrawer : SubDrawer {
    ...
    public override void DrawProp(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor) {
        ...
        if (extraPropName != "" && extraPropName != "_")
            p = URPGUI.FindProp(extraPropName, props, true);
...
public class ColorDrawer : SubDrawer {
    ...
    public override void DrawProp(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            var p = URPGUI.FindProp(colorStr[i - 1], props);
        ...
        for (var i = 1; i < count; i++) {
            ...
            var w = propRight * (0.8f + EditorGUI.indentLevel * 0.2f);
...
public class SubToggleDrawer : SubDrawer {
    ...
    public override void DrawProp(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor) {
        ...
        var k = Func.GetKeyWord(keyWord, prop.name);
        if (EditorGUI.EndChangeCheck()) {
