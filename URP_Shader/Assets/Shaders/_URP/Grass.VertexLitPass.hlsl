...
Varyings LitVert(Attributes input) {
    ...
    half3 viewDirWS = cameraPos - positionWS;
    ...
    shadowCoord = TransformWorldToShadowCoord(oriPositionWS);
#endif
    ...
    output.lambertAndFogFactor.rgb = Lambert(1, normalWS, shadowCoord, bakedGI, 0) + sss;
    //output.lambertAndFogFactor.rgb = sss;
...
half4 LitFragNoClip(Varyings input) : SV_Target {
    ...
    // color.rgb  =  input.lambertAndFogFactor.rgb;
    return color;
}
