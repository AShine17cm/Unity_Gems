#ifndef URP_VERTEXLIT_PASS_NEW_INCLUDED
#define URP_VERTEXLIT_PASS_NEW_INCLUDED
...
#if defined(_Need_ScreenPos)
    float2 pos = input.screenPos.xy/input.screenPos.w;
    clip(isDithered(pos,outSurfaceData.alpha));
#endif
    AlphaDiscard(alpha);
    half emissionMask = 1;
    half3 emission = SampleEmissionMask(uv, emissionMask);
...
half4 LitFragSkin(Varyings input) : SV_Target {
 ...
 float3 albedo1=lerp(albedo,albedo/_addSkinColor,step(emission.g,0.4980391f));
   //float3 albedo1=albedo/_addSkinColor;
  

#endif
    //albedo+=_addSkinColor.rgb;
    //albedo*=0.8;
    half3 matcap = 1;
#if defined(_MATCAP)
    SampleRGB(input.normalVS.xy, TEXTURE2D_ARGS(_MatcapMap, sampler_MatcapMap), matcap);
