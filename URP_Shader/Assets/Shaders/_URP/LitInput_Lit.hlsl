...
float RoughnessFunctionMapping(float _Smoothness)
{
    return _Smoothness > 0.633 ?  (0.4 * _Smoothness + 0.4): (1 / (1 + exp(-_Smoothness)));
}
inline void InitializeSurfaceDataPBR(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    outSurfaceData.smoothness *=  RoughnessFunctionMapping(_Smoothness);
