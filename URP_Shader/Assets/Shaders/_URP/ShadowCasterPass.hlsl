...
float3 ApplyShadowBias1(float3 positionWS, float3 normalWS, float3 lightDirection)
{
    ...
    float scale = invNdotL * _ShadowOffset.y;

    // normal bias is negative since we want to apply an inset normal offset
    positionWS = lightDirection * _ShadowOffset.xxx + positionWS;
...
Varyings ShadowPassVertex(Attributes input) {
    ...
    output.positionCS = TransformWorldToHClip(ApplyShadowBias1(positionWS, normalWS, _LightDirection));
    #if UNITY_REVERSED_Z
