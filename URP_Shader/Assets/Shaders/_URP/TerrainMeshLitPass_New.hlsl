#ifndef URP_TERRAIN_LIT_PASS_NEW_INCLUDED
#define URP_TERRAIN_LIT_PASS_NEW_INCLUDED
...
half4 SplatmapFragment_High(Varyings input) : SV_TARGET
{
    ...
    InitializeInputData(input, normalTS, inputData);
     //-------------------------------------
    half2 uv1=input.uv* _Caustics1_ST.xy + _Caustics1_ST.zw;
    half2 uv2=input.uv* _Caustics2_ST.xy + _Caustics2_ST.zw;
    uv1+=_Caustics1_Speed *_Time.y;
    uv2+=_Caustics2_Speed *_Time.y;
    half3 caustics1=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex,uv1);
    half3 caustics2=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex,uv2);
    half3 caustics=min(caustics2,caustics1);
    albedo+=caustics;
    half s=_CausticsSplitStrength;
    half r=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex , uv1 + half2(+s,+s)).r;
    half g=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex , uv1 + half2(+s,-s)).g;
    half b=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex , uv1 + half2(+s,+s)).b;
    half3 split1=half3(r,g,b);
    half r1=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex , uv2 + half2(+s,+s)).r;
    half g1=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex , uv2 + half2(+s,-s)).g;
    half b1=SAMPLE_TEXTURE2D(_CausticsTex,sampler_CausticsTex , uv2 + half2(+s,+s)).b;
    half3 split2=half3(r1,g1,b1);
    half3 split=min(split1,split2);
    albedo+=split;
    //----------------------------------------------
    half4 color = 1;
