Shader "_URP/UniversalLit"
{
    Properties
    {
        _Material_Quality("Material Quality", Float) = 0.0

        [ToogleOff] _ReceiveShadows("Receive Shadows", Float) = 1.0
        [HideInInspector] _ShaderType("ShaderType", Float) = 0.0
        [HideInInspector] _Surface("__surface", Float) = 0.0
        [HideInInspector] _Cull("__cull", Float) = 2.0
        [HideInInspector] _AlphaClip("__clip", Float) = 0.0
        _Cutoff("Alpha Clipping", Range(0.0, 1.0)) = 0.5
        [HideInInspector] _Blend("__blend", Float) = 0.0
        [HideInInspector] _SrcBlend("__src", Float) = 1.0
        [HideInInspector] _DstBlend("__dst", Float) = 0.0
        [HideInInspector] _ZWrite("__zw", Float) = 1.0
        _RenderSide("__Render_Side",Float) = 1.0

        [MainTexture] _BaseMap("Base Map (RGB) Occlusion (A)", 2D) = "gray" {}
        [NoScaleOffset] _NSMap("Normal Map (RGB), Smoothness(A)", 2D) = "gray" {}
        [NoScaleOffset]  _EmissionMap("Metallic(G) Emission(B) Noise(A)", 2D) = "gray" {}
        _SpecColor("Specular Color", Color) = (0.5, 0.5, 0.5, 0.5)
        _Smoothness("Smoothness", Float) = 0.5

     
        [Header(Emission)]
        _EmissionEnabled("__emission", Float) = 0.0
        [HDR]
        _EmissionColor ("Emission Color", Color) = (1, 1, 0, 0) 
        // Editor Field will add to _EmissionColor.a;
        _EmissionPower ("Emission Power", Range(0, 5)) = 1
        
        [Space]
        _PanOrPulsateEmission ("Pan Or Pulsate", Float ) = 0 
        _PanOrPulsate ("Pan: Tiling(xy) Speed(zw); Pulse: Speed(x), Power Range(yz), Color Variant(a)", Vector) = (1, 1, .5, 0)
        
        [Header(Pattern)]
        _EnableLit("Enable Lit", Float) = 0.0
        _EnablePattern("__pattern", Float) = 0.0
        _PatternMap("Pattern Map, Smoothness(A)", 2D) = "black" {}

        [Header(Hair)]
         _EnableHair("Enable Hair", Float) = 0.0
         _EnableGradient("EnableGradient",Float) = 0.0
         _GradientMap("Gradient Map", 2D) = "white" {}
         _Gradient_U_Speed("Gradient_U_Speed", Float) = 0
         _Gradient_V_Speed("Gradient_V_Speed", Float) = 0
    [HDR]_GradientColor("Gradient Color", Color) = (1, 1, 1, 1)
         _Fill("Fill",Range(-2,2)) = 0.5
         _Intensity("Intensity",Range(0,10)) = 0.5
        [KeywordEnum(Bitangent,Tangent)]
         _StrandDir("Strand Direction", Float) = 0
         _SpecularShift("Primary Specular Shift", Range(-1.0, 1.0)) = 0.1
    [HDR]_SpecularTint("Primary Specular Tint", Color) = (1, 1, 1, 1)
         _SpecularExponent("Primary Smoothness", Range(0.0, 1)) = .85
         _RimTransmissionIntensity("Rim Transmission Intensity", Range(0.0, 1.0)) = 0.5
         _AmbientReflection("Ambient Reflection Strength", Range(0.0, 1.0)) = 1
        [Header(Transmission)]
     
         _TranslucencyPower("Translucency Power", Range(0.0, 1.0)) = 1
         _ShadowStrength("Shadow Strength", Range(0.0, 1.0)) = 1
         _Distortion("Distortion", Range(0.0, 1.0)) = 1
        [Header(Skin)]
        _EnableSkinToggle ("EnableSkinBox", Float) = 0.0
        _ChangeColor("Skin Color", Color) = (0.7924528, 0.7924528, 0.7924528, 1)
        _ReadProps("__readprops", Float) = 0.0
        [NoScaleOffset] _LUTMap("Lut Map(RGB)", 2D) = "gray" {}
        _lightPower("lightPower",Range(1,2)) = 1.5
        _EnableSkin("EnableSkin", Float) = 0.0
        _addSkinColor("AddSkinColor",Color) = (0.7264151,0.7182957,0.6339)
        //stencil
        [IntRange] _StencilID("Stencil ID", Range(0,255)) = 0
        [Enum(UnityEngine.Rendering.CompareFunction)]_StencilCompMode("StencilCompMode",Float) = 3

        [ToggleOff] _SpecularHighlights("Specular Highlights", Float) = 1.0
        [ToggleOff] _EnvironmentReflections("Environment Reflections", Float) = 1.0
        
        // Editmode props
        [HideInInspector] _QueueOffset("Queue offset", Float) = 0.0
    }
        
    HLSLINCLUDE
    #define FOG_LINEAR 1
    #define _NORMALMAP 1
    #define _SPECULAR_SETUP 1
    #define _COLOR_SOFTLIGHT 1
    #pragma skip_variants _SHADOWS_SOFT DIRLIGHTMAP_COMBINED _MIXED_LIGHTING_SUBTRACTIVE
    #pragma skip_variants LIGHTMAP_ON _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHT_SHADOWS   
    #include "UniversalLitInput.hlsl"
    ENDHLSL
    SubShader
    {
        Tags { "RenderType" = "Opaque" "RenderPipeline" = "UniversalPipeline" "IgnoreProjector" = "True"}
        LOD 300
        
        Pass
        {
            Name "ForwardLit"
            Tags { "LightMode" = "UniversalForward" }
            Blend[_SrcBlend][_DstBlend]
            ZWrite[_ZWrite]
        
            Cull[_Cull]
            Stencil
            {
                Ref[_StencilID]
                Comp[_StencilCompMode]

            }
            HLSLPROGRAM
            // Required to compile gles 2.0 with standard srp library
            // All shaders must be compiled with HLSLcc and currently only gles is not using HLSLcc by default

            #pragma prefer_hlslcc gles
            #pragma exclude_renderers d3d11_9x
            #pragma target 2.0
            
            // -------------------------------------
            // Material Keywords
            
            #pragma shader_feature_local _EMISSION
            #if defined (_Emission)
            #pragma shader_feature_local _PAN
            #endif
            #pragma shader_feature_local _MASKMAP
            #pragma shader_feature_local _READ_PROPS
            #pragma shader_feature_local _PATTERNMAP
            #pragma shader_feature_local _HairGradient
            #pragma shader_feature_local _ENABLE_LIT
            #pragma shader_feature_local _ENABLE_HAIR
            #pragma shader_feature_local _ENABLE_SKIN
            #define _SCATTERING 1
            #if defined(_ENABLE_SKIN)
            #if defined(_READ_PROPS)
            #include "ShaderLibrary/SkinProps.hlsl"
            #else
            half3 _SubsurfaceColor;
            half _TranslucencyPower;
            half _ShadowStrength;
            half _Distortion;
            #endif 
            #endif 
  

            
           // #pragma shader_feature _NORMALMAP
            #if defined (_ENABLE_LIT) ||  defined (_ENABLE_HAIR)
            #pragma shader_feature_local _ALPHATEST_ON
            #endif
            #pragma shader_feature _RECEIVE_SHADOWS_OFF
            #pragma shader_feature _SPECULARHIGHLIGHTS_OFF
            #pragma shader_feature_local _ENVIRONMENTREFLECTIONS_OFF
            
            // -------------------------------------
            // Universal Pipeline keywords
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS_CASCADE
            #pragma multi_compile _ _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS
            #pragma multi_compile _ _ADDITIONAL_LIGHT_SHADOWS
            #pragma multi_compile _ _SHADOWS_SOFT
            #pragma multi_compile _ _MIXED_LIGHTING_SUBTRACTIVE
            
            // -------------------------------------
            // Unity defined keywords
            #pragma multi_compile _ DIRLIGHTMAP_COMBINED
            #pragma multi_compile _ LIGHTMAP_ON

            
           // #pragma multi_compile _ FOG_LINEAR
            
            //--------------------------------------
            // GPU Instancing
           // #pragma multi_compile_instancing

            #pragma vertex LitVert
            #pragma fragment LitFrag
         /*#if defined(_ENABLE_LIT)
            #pragma fragment LitFragLit
         #endif
         #if defined(_ENABLE_SKIN)
            #pragma fragment LitFragSkin
         #endif
         #if defined(_ENABLE_HAIR)
            #pragma fragment LitFragHair
         #endif*/
            #include "UniversalLitForwardPass.hlsl"
            ENDHLSL
        }
        
        Pass
        {
            Name "ShadowCaster"
            Tags{"LightMode" = "ShadowCaster"}

            ZWrite On
            ZTest LEqual
            Cull [_Cull]

            HLSLPROGRAM
            // Required to compile gles 2.0 with standard srp library
            #pragma prefer_hlslcc gles
            #pragma exclude_renderers d3d11_9x
            #pragma target 2.0
            
            // -------------------------------------
            // Material Keywords
            #pragma shader_feature_local _ALPHATEST_ON

            //--------------------------------------
            // GPU Instancing
            #pragma multi_compile_instancing

            #pragma vertex ShadowPassVertex
            #pragma fragment ShadowPassFragment
            
            #include "ShadowCasterPass.hlsl"
            ENDHLSL
        }
        
        UsePass "_URP/OnlyPass/DepthOnly"
        UsePass "_URP/OnlyPass/LitMeta"
    }
    
    SubShader
    {
        Tags { "RenderType" = "Opaque" "RenderPipeline" = "UniversalPipeline" "IgnoreProjector" = "True"}
        LOD 250

        Pass
        {
            Name "ForwardLit"
            Tags { "LightMode" = "UniversalForward" }
            
            Cull[_Cull]
            
            HLSLPROGRAM
            // Required to compile gles 2.0 with standard srp library
            // All shaders must be compiled with HLSLcc and currently only gles is not using HLSLcc by default

            #pragma prefer_hlslcc gles
            #pragma exclude_renderers d3d11_9x
            #pragma target 2.0
            
            // -------------------------------------
            // Material Keywords
            
            #pragma shader_feature_local _EMISSION
            #if defined (_Emission)
            #pragma shader_feature_local _PAN
            #endif
            #pragma shader_feature_local _MASKMAP
            #pragma shader_feature_local _PATTERNMAP
            #pragma shader_feature_local _HairGradient
            // #pragma shader_feature _NORMALMAP
            #if defined (_ENABLE_LIT) ||  defined (_ENABLE_HAIR)
            #pragma shader_feature_local _ALPHATEST_ON
            #endif
            #pragma shader_feature_local _ENABLE_LIT
            #pragma shader_feature_local _ENABLE_HAIR
            #pragma shader_feature_local _ENABLE_SKIN
            #pragma shader_feature _RECEIVE_SHADOWS_OFF
            #pragma shader_feature _SPECULARHIGHLIGHTS_OFF
            #pragma shader_feature_local _ENVIRONMENTREFLECTIONS_OFF
            
            // -------------------------------------
            // Universal Pipeline keywords
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS_CASCADE
            #pragma multi_compile _ _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS
            #pragma multi_compile _ _ADDITIONAL_LIGHT_SHADOWS
            #pragma multi_compile _ _SHADOWS_SOFT
            #pragma multi_compile _ _MIXED_LIGHTING_SUBTRACTIVE
            
            // -------------------------------------
            // Unity defined keywords
            #pragma multi_compile _ DIRLIGHTMAP_COMBINED
            #pragma multi_compile _ LIGHTMAP_ON
            //#pragma multi_compile _ FOG_LINEAR
            
            //--------------------------------------
            // GPU Instancing
            #pragma multi_compile_instancing

            #pragma vertex LitVert
          #if defined(_ENABLE_SKIN)
            #pragma fragment LitFragSkin
         #endif
         #if defined (_ENABLE_LIT) ||  defined (_ENABLE_HAIR)
            #pragma fragment LitFrag
         #endif  
            #include "UniversalSimpleLitForwardPass.hlsl"
            ENDHLSL
        }
        
        Pass
        {
            Name "ShadowCaster"
            Tags{"LightMode" = "ShadowCaster"}

            ZWrite On
            ZTest LEqual
            Cull [_Cull]

            HLSLPROGRAM
            // Required to compile gles 2.0 with standard srp library
            #pragma prefer_hlslcc gles
            #pragma exclude_renderers d3d11_9x
            #pragma target 2.0
            
            // -------------------------------------
            // Material Keywords
            #pragma shader_feature_local _ALPHATEST_ON

            //--------------------------------------
            // GPU Instancing
            #pragma multi_compile_instancing

            #pragma vertex ShadowPassVertex
            #pragma fragment ShadowPassFragment

            #include "ShadowCasterPass.hlsl"
            ENDHLSL
        }
        UsePass "_URP/OnlyPass/DepthOnly"
        UsePass "_URP/OnlyPass/Meta"
    }
    
    SubShader
    {
        Tags { "RenderType" = "Opaque" "RenderPipeline" = "UniversalPipeline" "IgnoreProjector" = "True"}
        LOD 200
        
        Pass
        {
            Name "ForwardLit"
            Tags { "LightMode" = "UniversalForward" }
            
            Cull[_Cull]
            HLSLPROGRAM
            
            // Required to compile gles 2.0 with standard srp library
            #pragma prefer_hlslcc gles
            #pragma exclude_renderers d3d11_9x
            #pragma target 2.0
            
            // -------------------------------------
            // Material Keywords
            // #pragma multi_compile MATERIAL_QUALITY_MEDIUM MATERIAL_QUALITY_LOW
            
            #pragma shader_feature_local _EMISSION
            #if defined (_Emission)
            #pragma shader_feature_local _PAN
            #endif

        
            #pragma shader_feature_local _PATTERNMAP        
            #if defined (_ENABLE_LIT) ||  defined (_ENABLE_HAIR)
            #pragma shader_feature_local _ALPHATEST_ON
            #endif
            #pragma shader_feature_local _ENABLE_LIT
            #pragma shader_feature_local _ENABLE_HAIR
            #pragma shader_feature_local _ENABLE_SKIN
            #pragma shader_feature _RECEIVE_SHADOWS_OFF
            #pragma shader_feature _SPECULARHIGHLIGHTS_OFF 
            
            // -------------------------------------
            // Universal Pipeline keywords
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS_CASCADE
            #pragma multi_compile _ _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS
            #pragma multi_compile _ _ADDITIONAL_LIGHT_SHADOWS
            #pragma multi_compile _ _SHADOWS_SOFT
            #pragma multi_compile _ _MIXED_LIGHTING_SUBTRACTIVE
            
            // -------------------------------------
            // Unity defined keywords
            #pragma multi_compile _ DIRLIGHTMAP_COMBINED
            #pragma multi_compile _ LIGHTMAP_ON
           // #pragma multi_compile _ FOG_LINEAR
            
            //--------------------------------------
            // GPU Instancing
            #pragma multi_compile_instancing

            #pragma vertex LitVert
         #if defined(_ENABLE_SKIN)
            #pragma fragment LitFragSkin
         #endif
         #if defined (_ENABLE_LIT) ||  defined (_ENABLE_HAIR)
            #pragma fragment LitFrag
         #endif  
      
            #include "UniversalLambertLitPass.hlsl"
            ENDHLSL
        }
        
        Pass
        {
            Name "ShadowCaster"
            Tags{"LightMode" = "ShadowCaster"}

            ZWrite On
            ZTest LEqual
            Cull [_Cull]

            HLSLPROGRAM
            // Required to compile gles 2.0 with standard srp library
            #pragma prefer_hlslcc gles
            #pragma exclude_renderers d3d11_9x
            #pragma target 2.0
            
            // -------------------------------------
            // Material Keywords
            #pragma shader_feature_local _ALPHATEST_ON

            //--------------------------------------
            // GPU Instancing
            #pragma multi_compile_instancing

            #pragma vertex ShadowPassVertex
            #pragma fragment ShadowPassFragment

            #include "ShadowCasterPass.hlsl"
            ENDHLSL
        }
        UsePass "_URP/OnlyPass/DepthOnly"
        UsePass "_URP/OnlyPass/VertexLitMeta"
    }
    FallBack "Hidden/Universal Render Pipeline/FallbackError"
    CustomEditor "URP.Editor.UniversalLitShaderGUI"
}
