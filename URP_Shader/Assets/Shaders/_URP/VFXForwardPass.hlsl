...
#ifndef URP_VFX_PASS_INCLUDED
#define URP_VFX_PASS_INCLUDED
...
half4 Frag(VertexOutput input): SV_Target
{
	...
	float2 panner_MainTex=PolarAndRotator(tempAppenduv,_MainUVSpeedAndRota.z,_MainUVSpeedAndRota.x,_MainUVSpeedAndRota.y);
	...
	#endif
	#else
	float2 panner_MainTex=PolarAndRotator(uvMainTex,_MainUVSpeedAndRota.z,_MainUVSpeedAndRota.x,_MainUVSpeedAndRota.y);
	float4 appendnode=SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,(panner_MainTex + SAMPLE_TEXTURE2D(_PannerTex,sampler_PannerTex, panner_PannerTex).r * lerpuv4.w));
	float4 lerpResult=lerp(_EdgeColor,(_BaseColor * appendnode * input.color),saturate(((disTexR-(temp_output * (1.0 + oneSubHardNess)))-_Hardness)/oneSubHardNess));
	#ifdef _FresnelBlend_On
	...
	#endif
	#ifdef _FresnelBlend_On
	float Alpha=saturate(saturate(((disTexR-(temp_output - _Edgewidth)*(1.0 + oneSubHardNess))-_Hardness)/oneSubHardNess) * _BaseColor.a * appendnode.a * input.color.a * maskNode.r *maskNode.a * _MaskUVSpeedAndRota.w);
	#else
	float Alpha=saturate(((1-(temp_output - _Edgewidth)*(1.0 + oneSubHardNess))-_Hardness)/oneSubHardNess);
	#endif
	float AlphaClipThreshold = 0.5;
